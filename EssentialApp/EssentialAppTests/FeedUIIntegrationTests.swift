...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_loadMoredActions_requestMoreFromLoader() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        ...
        XCTAssertEqual(loader.loadMoreCallCount, 1, "Expected a load more requests")
        ...
        sut.simulateLoadMoreFeedAction()
        XCTAssertEqual(loader.loadMoreCallCount, 3, "Expected request after load more failed")
    ...
    func test_feedLoadingIndicator_isVisibleWhileLoadingFeed() {
        let (sut, loader) = makeSUT()
    ...
    func test_feedImageView_rendersImageLoadedFromURL() {
        ...
        XCTAssertEqual(view0?.renderedImage, imageData0, "Expected image for first view once first image completes successfully")
        XCTAssertEqual(view1?.renderedImage, .none, "Expected no image state change for second view once first image loading completes successfully")

        let imageData1 = UIImage.make(withColor: .blue).pngData()!
        loader.completeImageLoading(with: imageData1, at: 1)
        XCTAssertEqual(view0?.renderedImage, imageData0, "Expected no image state change for first view once second image loading completes successfully")
        XCTAssertEqual(view1?.renderedImage, imageData1, "Expected image for second view once second image loading complets successfully")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnImageURLLoadError() {
        ...
        XCTAssertEqual(view1?.isShowingRetryAction, true, "Expected retry action for second view once second image loading completes with errro")
    }
    ...
    func test_feedImageViewRetryButton_isVisibleOnInvalidImageData() {
        ...
        XCTAssertEqual(view?.isShowingRetryAction, false, "Expected to retry action while loading image")
    ...
    func test_feedImageViewRetryAction_retriesImageLoad() {
        ...
        XCTAssertEqual(loader.loadedImageURLs, [image0.url, image1.url, image0.url], "Expected thrid imageURL request after first view retry action")
    ...
    func test_feedImageView_doesNotRenderLoadedImagWhenNotVisibleAnymore() {
        let (sut, loader) = makeSUT()
        sut.loadViewIfNeeded()
        loader.completeFeedLoading(with: [makeImage()])
